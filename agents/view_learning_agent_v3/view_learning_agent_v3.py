...
class ViewLearningAgentV3(BaseAgent):
    ...
    def __init__(self, server_url: str, mode="train"):
        ...
        try:
            with open('./view_learning_agent_v3/learned_data.pkl', 'rb') as f:
    ...
    def rebuild_memory_after_portals(self, new_position):
        ...
        explored_cells = {(exp[0] + delta[0], exp[1] + delta[1]) for exp in self.explored_cells if round(exp[0] / 10000) == region}
        explored_cells.update({exp for exp in self.explored_cells if round(exp[0] / 10000) != region})
        reachable_cells = {(exp[0] + delta[0], exp[1] + delta[1]) for exp in self.reachable_cells if round(exp[0] / 10000) == region}
        reachable_cells.update({exp for exp in self.reachable_cells if round(exp[0] / 10000) != region})
    ...
    def create_training_data(self):
        ...
        training_data_files = os.listdir('./view_learning_agent_v3/training_data')
        if len(training_data_files) == len(sizes) * len(seeds):
            return
        else:
            shutil.rmtree('./view_learning_agent_v3/training_data')
            os.mkdir('./view_learning_agent_v3/training_data')
            for size in sizes:
                for seed in seeds:
                    with open(f'./view_learning_agent_v3/training_data/{size[0]}x{size[1]}_{seed}.png', 'wb') as f:
                        try:
                            image = requests.post(f'{self.server_url}/maze/', json={"seed": seed, "width": size[0], "height": size[1]}).content
                            f.write(image)
                        except:
                            print("Error fetching image")
                            os.rmdir('./view_learning_agent_v3/training_data')
                            return
    ...
    def move_to_target(self, target: tuple):
        ...
        directions = {'N': (-1, 0), 'E': (0, 1), 'S': (1, 0), 'W': (0, -1), 'P': (0, 0)}  # Possible directions
        reverse_directions = {(-1, 0): "N", (0, 1): "E", (1, 0): "S", (0, -1): "W", (0, 0): "P"}  # Reverse directions
        ...
        while open_list:
            # Get the node with the lowest f(n) (priority)
            ...
            if current == end:
                ...
                for m in range(len(path) - 1):
                    if self.memory[path[m]] in range(150, 170):
                        portaled = False
                        for portal in self.portal_positions[self.memory[path[m]]]:
                            if portal != path[m] and portal == path[m + 1]:
                                result += "P"
                                portaled = True
                                break
                        if portaled:
                            continue
                    if (path[m + 1][0] - path[m][0], path[m + 1][1] - path[m][1]) in reverse_directions:
                        result += reverse_directions[(path[m + 1][0] - path[m][0], path[m + 1][1] - path[m][1])]
                    else:
                        signx = 1 if (path[m + 1][0] - path[m][0]) > 0 else -1
                        signy = 1 if (path[m + 1][1] - path[m][1]) > 0 else -1

                        result += reverse_directions[(signx * min(1, abs(path[m + 1][0] - path[m][0])), signy * min(1, abs(path[m + 1][1] - path[m][1])))]
                return result

            # Explore neighbors
            for direction in directions:
                neighbor = (current[0] + directions[direction][0], current[1] + directions[direction][1])
                if neighbor not in self.memory or self.memory[neighbor] == 0 or self.memory[neighbor] in range(101, 106):
                    continue
                elif self.memory[neighbor] in range(106, 116):
                    while neighbor in self.memory and self.memory[neighbor] in range(106, 116):
                        if self.memory[neighbor] in range(106, 111):
                            n = self.memory[neighbor] - 105
                            neighbor = (neighbor[0] + n * directions[direction][0], neighbor[1] + n * directions[direction][1])
                        else:
                            n = self.memory[neighbor] - 110
                            neighbor = (neighbor[0] - n * directions[direction][0], neighbor[1] - n * directions[direction][1])
                    if neighbor not in self.memory or self.memory[neighbor] == 0 or self.memory[neighbor] in range(101, 106):
                        continue
                elif self.memory[neighbor] in range(150, 170) and direction == "P":
                    for portal in self.portal_positions[self.memory[neighbor]]:
                        if portal != neighbor:
                            neighbor = portal
                            break
                ...
                # If this path to neighbor is shorter or neighbor is unvisited
    ...
    def train(self):
        ...
        for file in training_data_files:
            with open(f'./view_learning_agent_v3/training_data/{file}', 'rb') as f:
        ...
        with open('./view_learning_agent_v3/learned_data.pkl', 'wb+') as f:
    ...
    def move_score(self, current: tuple, move: str):
        ...
        if current not in self.memory:
            ...
            score = -1
        elif self.memory[current] == 0:
            score = 0
        elif self.memory[current] in range(106, 111):
            n = self.memory[current] - 105
            current = (current[0] + n * moves[move][0], current[1] + n * moves[move][1])
            if current not in self.memory or self.memory[current] == 0 or self.memory[current] in range(101, 106):
                return score, current
            elif current not in self.explored_cells:
                dest_result, current = self.move_score(current, move)
                if dest_result > 0:
                    score = dest_result + 1
                else:
                    score = dest_result
        elif self.memory[current] in range(111, 116):
            n = self.memory[current] - 110
            current = (current[0] - n * moves[move][0], current[1] - n * moves[move][1])
            if current not in self.memory or self.memory[current] == 0 or self.memory[current] in range(101, 106):
                return score, current
            elif current not in self.explored_cells:
    ...
    def calculate_path_score(self, path: list, position: tuple):
        ...
        for move in path:
            ...
            result, current = self.move_score(current, move)
            if result == 0:
                return score
            if result < 0:
                return result
    ...
    def move(self):
        ...
        if self.position in self.reachable_cells:
            self.reachable_cells.remove(self.position)
        ...
        if best_score <= 0:
            if self.fallback_target is None:
                closest_reachable = None if len(self.reachable_cells) == 0 else min(self.reachable_cells, key=lambda x: self.heuristic(x, self.position))
                ...
                while best_path is None:
                    tried_targets.add(closest_reachable)
                    if tried_targets == self.reachable_cells:
                        print("No path to any reachable neighbor")
                        return "".join([random.choice(["N", "E", "S", "W"]) for _ in range(self.moves)])
                    closest_reachable = min(self.reachable_cells - tried_targets, key=lambda x: sys.maxsize * 2 + 1 if closest_reachable in tried_targets else self.heuristic(x, self.position))
                if best_path is None:
                    print("No path to neighbor", closest_reachable)
                    return "".join([random.choice(["N", "E", "S", "W"]) for _ in range(self.moves)])
                self.fallback_target = closest_reachable
                self.fallback_path = best_path

                best_path = best_path[:self.moves]
                self.fallback_path = self.fallback_path[self.moves:]
                ...
                if self.fallback_path is None:
                    print("No path to neighbor", self.fallback_target)
                    return "".join([random.choice(["N", "E", "S", "W"]) for _ in range(self.moves)])
                best_path = self.fallback_path[:self.moves]
                self.fallback_path = self.fallback_path[self.moves:]
                if self.fallback_path == "":
                    self.fallback_target = None
                    self.fallback_path = None
